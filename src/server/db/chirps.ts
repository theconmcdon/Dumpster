...
const all = async () => Query(`
    select chirps.id, chirps.text, chirps.location, chirps.day, chirps.time, users.name, users.email
    from chirps
    join users on chirps.userid = users.id;
`);
const one = async (id: any) => Query(`
    select chirps.text, chirps.location, chirps.day, chirps.time, users.name, users.email
    from chirps
    join users on chirps.userid = users.id
    where chirps.id = ?;
`, [id]);
...
const put = async (id: string, newText: string) => Query(`
    UPDATE chirps
    SET text = ?
    WHERE chirps.id = ?;
`, [newText, id]);
